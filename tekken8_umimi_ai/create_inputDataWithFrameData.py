...
with open("input_data.txt", 'w') as input_file:
	...
	for data in holder_list:
		data = [int(value) for value in data.split(',')]  # converts string to int IMPORTANT

		pressed_notations = [input_index_position[i] for i in range(len(data) - 1) if data[i] == 1]
		...
		previous_notation_str = '-'.join(last_input)
		notation_str = '-'.join(pressed_notations)

		if frame_count == 1:
			input_file.write(previous_notation_str + "\t frames: " + str(previous_frame_count) + '\n')
		# input_file.write(notation_str + "\t frames: " + str(frame_count) + '\n')

		# Record last input at the very end
		last_input = pressed_notations
		previous_frame_count = data[10]
